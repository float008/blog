...
import { pickLocale, siteConfig, type TimelineItem } from "@/lib/site-config";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: t("title"),
    description: pickLocale(siteConfig.bio, locale),
  };
}
    ...
    <div className="mx-auto max-w-3xl px-4 py-16">
      ...
      <Reveal className="flex flex-col items-start gap-6 sm:flex-row sm:items-center">
        <Avatar className="size-20 ring-2 ring-primary/20">
          <AvatarImage src={siteConfig.avatarUrl} alt={name} />
      ...
      >
        {pickLocale(siteConfig.bio, locale)}
      </Reveal>
      ...
      <Reveal className="mt-12">
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          <InfoRow
            label={t("location")}
            value={pickLocale(siteConfig.location, locale)}
          />
          <InfoRow
            label={t("yearsOfExperience")}
            value={t("yearsValue", { years: siteConfig.yearsOfExperience })}
          />
          <InfoRow label={t("currentRole")} value={title} />
      ...
      <Reveal className="mt-12">
        ...
        <Timeline
          items={localizeTimeline(siteConfig.experience, locale)}
          className="mt-4"
        />
      </Reveal>
      ...
      <Reveal className="mt-12">
        ...
        <Timeline
          items={localizeTimeline(siteConfig.education, locale)}
          className="mt-4"
        />
      </Reveal>
      ...
      <Reveal className="mt-12">
        ...
        <Stagger stagger={0.1} className="mt-4 space-y-6">
          {siteConfig.skills.map((group, i) => {
            const category = pickLocale(group.category, locale);
            return (
...
// 把 site-config 里 Localized 字段批量结算成当前 locale 的纯字符串，方便传给 Timeline
...
function localizeTimeline(
  ...
  return items.map((item) => ({
    period: pickLocale(item.period, locale),
    title: pickLocale(item.title, locale),
    subtitle: pickLocale(item.subtitle, locale),
  }));
}
